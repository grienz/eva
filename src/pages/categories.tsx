              ...
              sortedModels.map((model) => (
                <div key={model.modelSlug} className="mt-2 mb-2 mr-5">
                  <Link href={`/model/${model.modelSlug}`}>
                    <a className="mx-2 my-2 flex text-base font-medium text-teal-600 transition-all delay-100 hover:text-teal-800 dark:hover:text-teal-400">
                      <AvatarImage
                        url={model.modelPicture}
                        alt={model.modelName}
                      />
                      <span className="px-1 py-1">{`${model.modelName} (${model.relatedProductsCount})`}</span>
                    </a>
                  </Link>
                </div>
              ))}
                ...
                <div key={tag.tagSlug} className="mt-2 mb-2 mr-5">
                  <Link href={`/tag/${tag.tagSlug}`}>
                    <a className="mx-2 my-2 flex text-base font-medium  text-teal-600 transition-all delay-100 hover:text-teal-800 dark:hover:text-teal-400">
                      ...
                      <span className="px-1 py-1">{`${tag.tagName} (${tag.relatedProductsCount})`}</span>
                    </a>
                  </Link>
                </div>
              ))}
